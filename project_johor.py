...
try:
    ...
    Select(driver.find_element(By.ID, "state")).select_by_value("01")
    time.sleep(2)
    ...
    for page in range(1, get_total_pages() + 1):
        ...
        for row in soup.select("table tbody tr"):
            try:
                ...
                log_error(f"Error processing row on page {page}: {str(e)}\n{traceback.format_exc()}")

except Exception as e:
    log_error(f"Critical error in main script: {str(e)}\n{traceback.format_exc()}")
finally:
    cursor.close()
    conn.close()
    driver.quit()
